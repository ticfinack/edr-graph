...
_MACOS_SYSTEM_BASELINE: frozenset[str] = frozenset({
    # Core system
    "kernel", "launchd", "kernel_task",
    # Display / UI
    "WindowServer", "Dock", "SystemUIServer", "ControlCenter",
    "NotificationCenter", "Spotlight", "loginwindow",
    "WallpaperAerialsExtension", "WallpaperVideoExtension",
    # Process management
    "runningboardd", "dasd", "distnoted", "cfprefsd", "lsd",
    "trustd", "syspolicyd", "endpointsecurityd",
    # Storage / Filesystem
    "corespotlightd", "spotlightknowledged", "mds", "mds_stores",
    "mdworker", "mdworker_shared", "fseventsd", "filecoordinationd",
    "fileproviderd", "revisiond",
    # Networking
    "mDNSResponder", "configd", "symptomsd", "networkd", "nsurlsessiond",
    ...
    "Reminders", "Maps", "Photos", "Music", "TV", "Podcasts",
    "News", "Stocks", "Home", "Books", "Preview", "TextEdit",
    "Terminal", "Activity Monitor", "System Preferences",
    "System Settings", "App Store", "FaceTime",
    # WebKit / browser helpers
    "com.apple.WebKit.Networking", "com.apple.WebKit.WebContent",
    "com.apple.WebKit.GPU", "com.apple.Safari.WebHosting",
    "SafariBookmarksSyncAgent",
    # System agents
    "UserEventAgent", "contextstored", "duetexpertd",
    "knowledge-agent", "progressd", "remindd",
    "CalendarAgent", "AddressBookSourceSync",
    "containermanagerd", "containermanager",
    "siriknowledged", "assistantd", "parsecd",
    "suggestd", "searchpartyd", "rapportd",
    "sharingd", "coreservicesd", "iconservicesagent",
    # Other background daemons
    "logd", "watchdogd", "mediaanalysisd", "photoanalysisd",
    "gamepolicyd", "gamecontrollerd", "donotdisturbd",
    "UIKitSystem", "ViewBridgeAuxiliary", "talagent",
    "AMPDeviceDiscoveryAgent", "AMPLibraryAgent",
    "CategoriesService", "analyticsagent",
    # Creative Cloud (common on developer machines)
    "Creative Cloud Helper", "Creative Cloud UI Helper (Renderer)",
    "Creative Cloud Libraries Synchronizer", "Core Sync",
    "CCXProcess", "CCLibrary", "AdobeIPCBroker",
...
_LINUX_SYSTEM_BASELINE: frozenset[str] = frozenset({
    ...
    "init", "kthreadd", "dbus-daemon", "polkitd", "rsyslogd",
    "cron", "atd", "NetworkManager", "wpa_supplicant",
    "avahi-daemon", "cupsd", "accounts-daemon",
    "snapd", "packagekitd", "udisksd", "thermald",
    "irqbalance", "acpid", "ModemManager",
    "gdm", "gdm-session-worker", "gnome-shell", "gnome-session",
...
_WINDOWS_SYSTEM_BASELINE: frozenset[str] = frozenset({
    ...
    "sihost.exe", "fontdrvhost.exe", "WmiPrvSE.exe",
    "dllhost.exe", "msiexec.exe", "TiWorker.exe",
...
def _check_process_novelty(
    conn: kuzu.Connection, event: ProcessActivity, threshold: int
) -> bool:
    ...
    result = conn.execute(
        "MATCH (u:User)-[s:SPAWNED]->(p:Process) "
        "WHERE p.name = $proc_name RETURN count(s) AS cnt",
        {"proc_name": proc_name},
    )
    if result.has_next():
...
def _check_network_novelty(
    conn: kuzu.Connection, event: NetworkActivity, threshold: int
) -> bool:
    ...
    result = conn.execute(
        "MATCH (p:Process {name: $proc})-[c:CONNECTED_TO]->(ip:IP {id: $ip}) "
...
def _check_auth_novelty(
    conn: kuzu.Connection, event: Authentication, threshold: int
) -> bool:
